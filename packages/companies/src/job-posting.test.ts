...
describe('jobPostingFromListing', () => {
  ...
  it('recruit-page: opts で社名と kind を固定する', () => {
    ...
    const item = jobPostingFromListing('PrivateProject-career', 'https://PrivateProject.com/career/',
      entry({ title: '3Dアーティスト', companyName: '' }),
      { companyName: '株式会社PrivateProject', kind: 'recruit-page' });
    expect(item!.kind).toBe('recruit-page');
    expect(item!.companyName).toBe('株式会社PrivateProject');
    // dedupKey の合成にも固定社名が乗る (normalizeUrl は末尾スラッシュを落とす)。
    expect(item!.dedupKey).toBe('https://PrivateProject.com/career#3Dアーティスト@株式会社PrivateProject');
  });
