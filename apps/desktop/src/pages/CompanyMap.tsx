...
import { useCompaniesApi, type MapMarker } from '../api/companies.js';
  ...
  const addMarkers = (markers: MapMarker[]) => {
    ...
    for (const m of markers) {
      ...
      marker.addListener('click', () => {
        const chips = [m.is_smb ? '中小' : '大手', m.is_social ? 'ソシャゲ' : '', `ゲーム${m.game_count}本`]
          .filter(Boolean)
          .join(' / ');
        infoObj.current.setContent(
          `<div style="font-size:13px;line-height:1.6"><strong>${m.name}</strong><br>${m.location}<br>${chips}</div>`,
        );
